...
void uvRipple(inout vec2 uv, float intensity){
    ...
    float cLength=length(p);
     ...
     uv= uv +(p/cLength)*cos(cLength*15.0-vTime*4.0)*intensity;
...
void main(){
  ...
  //
  // uvRippleStatic(color.rb, .9 );
  // uvRippleStatic(color.gb, .09 );
  // uvRipple(uv, .09 );
  // pMod2(color.gb, vec2(.5));
  vec2 rote = rotateUV(uv1, vec2(.5), PI * vTime * .05);
  vec2 roteC = rotateUV(uv1, vec2(.5), -PI * vTime * .05);


  color = mix(color, 1.-color, stroke(raysSDF(uv, 15. ), .2 , .8));

  // color = mix(color, 1.-color,uv.x);
  color +=(stroke(polySDF(uv1, 10. ), .5, .2));


  vec2 uv2 = uv * 4.0 *  cnoise(vec2(uv *rot(vTime * .1)) * 20.) ;

  color = vec3(uv2.x, uv2.y, cnoise(vec2(uv2 *rot(vTime * .1)) * 10.));
  color.xyz += 8. * .1 * cos(3. * color.yzx + vTime * .5) ;
